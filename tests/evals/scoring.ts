...
export function exactMatch(keywords: string[]): (result: AgentResult) => number {
...
export function stepCount(targetSteps: number): (result: AgentResult) => number {
  return (result: AgentResult) => {
...
export function tokenUsage(tokenBudget: number): (result: AgentResult) => number {
  return (result: AgentResult) => {
